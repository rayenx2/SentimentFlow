...
<p align="center">
  <img src="assets/banner.svg?v=2" alt="SentimentFlow Banner" width="800"/>
</p>
...
**Live:** [https://sentimentflow-demo.vercel.app](https://sentimentflow-demo.vercel.app)
...
<p align="center">
  <img src="assets/architecture.svg" alt="SentimentFlow Architecture" width="700"/>
</p>
...
```bash
# Clone the repo
git clone git@github.com:Hamilas/SentimentFlow.git
cd SentimentFlow
...
> MLflow is included as its own Docker Compose service (`docker compose up -d mlflow`, UI at `http://localhost:5000`) and isn't part of the core stack by default since it's only needed when tracking evaluation runs. The codebase has an MLflow tracking module (`app/tracking/mlflow_tracker.py`) with graceful fallback: if no MLflow server is reachable, predictions continue normally. Set `MLFLOW_TRACKING_URI` in `.env` to point at it.
