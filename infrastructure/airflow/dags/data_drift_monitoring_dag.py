...
def sample_reference_data(ds, **kwargs):
    ...
    try:
        ...
        hf_ds = hf_load_dataset(dataset_name, "sentiment", trust_remote_code=True)

        split = "test" if "test" in hf_ds else "train"
        data_split = hf_ds[split]
        text_column, label_column = "text", "label"

        total = len(data_split)
        indices = random.sample(range(total), min(config["reference_samples"], total))
        sampled = data_split.select(indices)

        reference_data = {
            "texts": sampled[text_column],
            "labels": sampled[label_column] if label_column in sampled.features else [],
            "text_lengths": [len(t) for t in sampled[text_column]],
        ...
        logging.info(f"Reference data sampled: {len(sampled)} samples from {dataset_name}")
        return {"status": "success", "dataset": dataset_name, "samples": len(sampled), "file_path": reference_file}
...
def sample_current_data(ds, **kwargs):
    ...
    try:
        ...
        total = len(data_split)
        indices = random.sample(range(total), min(config["current_samples"], total))
        sampled = data_split.select(indices)
        ...
        logging.info(f"Current data sampled: {len(sampled)} samples from {dataset_name}")
        return {"status": "success", "dataset": dataset_name, "samples": len(sampled), "file_path": current_file}
